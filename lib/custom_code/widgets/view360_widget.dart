...
class View360Widget extends StatefulWidget {
  const View360Widget({
    super.key,
    this.width,
    this.height,
  ...
  @override
  State<View360Widget> createState() => _View360WidgetState();
}

class _View360WidgetState extends State<View360Widget> {
  int _currentIndex = 0;
  String idPrincipal = '';
  ...
  void initState() {
    super.initState();
    varNivelSelect = FFAppState().stateNivelid;
    varidVirtualTour = FFAppState().globalVar2;
  }
                      ...
                      if (_isPickingImages)
                        Center(
                          child: Stack(
                            children: [
                              const SizedBox(
                                  height: 158), // Add spacing between buttons
...
Future<List<ImageFile>> pickImagesUsingImagePicker(bool allowMultiple) async {
  ...
  if (pickedFiles != null && pickedFiles.isNotEmpty) {
    final uploadedImages = await Future.wait(pickedFiles.map((file) async {
      showLoadingDialog(); // Reemplazar con tu función de mostrar loader
      ...
      hideLoadingDialog(); // Reemplazar con tu función de ocultar loader
