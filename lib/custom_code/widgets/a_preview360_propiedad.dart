...
class APreview360Propiedad extends StatefulWidget {
  const APreview360Propiedad({
    super.key,
    this.width,
    this.height,
    this.paramReference,
  ...
  @override
  State<APreview360Propiedad> createState() => _APreview360PropiedadState();
}

class _APreview360PropiedadState extends State<APreview360Propiedad> {
  int _currentIndex = 0;
  ...
  Future<void> fetchPrincipalTourId() async {
    await getPrincipalTourId();
    inicioFunction();
    setState(() {
      _isLoading = false;
    });
  ...
  Future<String?> getPrincipalTourId() async {
    try {
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance
          .collection('virtualTours')
          .doc(widget.paramReference)
          .collection('tours')
          .where('isPrincipal', isEqualTo: true)
          .get();

      if (querySnapshot.docs.isNotEmpty) {
        print('isPrincipal:' + querySnapshot.docs.first.id);
        idTourInit = querySnapshot.docs.first.id;
        return querySnapshot.docs.first.id;
      } else {
        return null;
      }
    } catch (e) {
      print('Error fetching principal tour ID: $e');
      return null;
    }
  }
  ...
  void addHotspot(String idTourHot, double lat, double lon, String zLink) {
    setState(() {
  ...
  Future<List<Map<String, dynamic>>> getVirtualTourData(
    ...
    try {
      ...
      if (docSnapshot.exists) {
        ...
        print('idDocTour:' + idDocTour.toString());
        // Limpia hotspots antes de añadir los nuevos
        hotspotsArray.clear();
        ...
        final processedHotspots = hotspots.map((hotspot) {
          ...
          addHotspot(hotspot['idTour'], hotspot['lat'], hotspot['lon'],
              hotspot['zLink']);
